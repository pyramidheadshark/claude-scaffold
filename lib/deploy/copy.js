...
const HOOKS_DEFINITION = require('../hooks-definition.json');
...
function copyDirContents(srcDir, dstDir, ext) {
  ...
  for (const entry of fs.readdirSync(srcDir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      copyDirContents(srcPath, dstPath, ext);
    } else {
...
function generateSkillRules(infraDir, targetDir, skills) {
  ...
  let sourceRules;
  try {
    sourceRules = JSON.parse(fs.readFileSync(srcPath, 'utf8'));
  } catch (e) {
    throw new Error(`Cannot read skill-rules.json from infra at ${srcPath}: ${e.message}`);
  }
