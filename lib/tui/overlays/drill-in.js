...
function createDrillInOverlay(screen, blessed, agentPool, agentId, setStatus) {
  ...
  const overlay = blessed.box({
    ...
    label: ` {yellow-fg}Agent #${agentId}{/yellow-fg}: ${agent.name}  {gray-fg}[ESC to exit]{/gray-fg} `,
    tags: true,
    scrollable: true,
    alwaysScroll: true,
  ...
  function syncContent() {
    overlay.setContent(processAnsi(agent.output));
  ...
  function keypressHandler(ch, key) {
    ...
    if (key.full === 'C-c' || (key.ctrl && key.name === 'c')) {
